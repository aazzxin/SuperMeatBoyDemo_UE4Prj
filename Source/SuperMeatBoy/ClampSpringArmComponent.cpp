...
void UClampSpringArmComponent::BeginPlay() {
	Super::BeginPlay();
}

void UClampSpringArmComponent::UpdateDesiredArmLocation(bool bDoTrace, bool bDoLocationLag, bool bDoRotationLag, float DeltaTime) {
	...
	// 添加限制，DesiredLoc只能在关卡范围内
	DesiredLoc.X = FMath::Clamp<float>(DesiredLoc.X, LevelBoundXMin, LevelBoundXMax);
	DesiredLoc.Z = FMath::Clamp<float>(DesiredLoc.Z, LevelBoundZMin, LevelBoundZMax);
